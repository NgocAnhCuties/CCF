...
export const synchronizeZeroTrustLists = async (items) => {
  ...
  if (toAdd.length) {
    ...
    const nextListNumber = Math.max(0, ...cgpsLists.map(list => parseInt(list.name.replace('CGPS List - Chunk ', ''), 10)).filter(x => Number.isInteger(x))) + 1;
...
export const defragmentZeroTrustLists = async () => {
  ...
  cgpsLists.sort((a, b) => {
    const aNum = parseInt(a.name.replace("CGPS List - Chunk ", ""), 10);
    const bNum = parseInt(b.name.replace("CGPS List - Chunk ", ""), 10);
    return aNum - bNum;
  });
